...
import 'package:zipcular/models/map/manager/cluster.dart';
import 'package:zipcular/models/map/manager/max_dist_clustering.dart';
...
class ClusterManager<T extends ClusterItem> {
  ClusterManager(this._items, this.updateMarkers,
      {Future<GoogleMaps.Marker> Function(Cluster<T>)? markerBuilder,
  ...
  final Future<GoogleMaps.Marker> Function(Cluster<T>) markerBuilder;
  ...
  final void Function(Set<GoogleMaps.Marker>) updateMarkers;
  ...
  void setMapId(int mapId, {bool withUpdate = true}) async {
    _mapId = mapId;
    _zoom = await GoogleMaps.GoogleMapsFlutterPlatform.instance
  ...
  void onCameraMove(GoogleMaps.CameraPosition position, {forceUpdate = false}) {
    _zoom = position.zoom;
    if (forceUpdate) {
      updateMap();
    }
  ...
  Future<List<Cluster<T>>> getMarkers() async {
    ...
    final GoogleMaps.LatLngBounds mapBounds = await GoogleMaps
        .GoogleMapsFlutterPlatform.instance
        .getVisibleRegion(mapId: _mapId!);

    late GoogleMaps.LatLngBounds inflatedBounds;
  ...
  GoogleMaps.LatLngBounds _inflateBounds(GoogleMaps.LatLngBounds bounds) {
    ...
    return GoogleMaps.LatLngBounds(
      southwest: GoogleMaps.LatLng(bounds.southwest.latitude - lat, wLng),
  ...
  static Future<GoogleMaps.Marker> Function(Cluster) get _basicMarkerBuilder =>
      (cluster) async {
        return GoogleMaps.Marker(
          markerId: GoogleMaps.MarkerId(cluster.getId()),
  ...
  static Future<GoogleMaps.BitmapDescriptor> _getBasicClusterBitmap(int size,
      {String? text}) async {
